...
├── preregistrations/    the six executed pre-registrations (+ multivariate arm) + synthesis
...
| 6 | [experiment_plan_density_invariance.md](preregistrations/experiment_plan_density_invariance.md) | Density-model invariance of the support-widening dilemma: **H1c SUPPORTED 4/4** (no shared clean-tail boundary reaches fold-mean FAR < 2% with extent-1 absorption < 50% under GMM deficit, PCA-SPE, T², or calibrated combined MSPC — the dilemma is formulation-level); **H2 SUPPORTED 4/4** (E3 commissioning restores admission 0.10–0.34% under every family, bit-identical severity); **H3 first half KILLED — K3 fires**: the severity ladder is likelihood-deficit-specific (M2/M3/M5 break or compress it; FGMM-BIP saturates at 1.0000); GMM cells are disclosed replication anchors (K4 exact integer match) |
...
The six pre-registered validations:
...
python -m tests.paderborn.exp_density_invariance # #6
```
